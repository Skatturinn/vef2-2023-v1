...
import { table_template, template } from './html.js';
...
if (indexfile) {
	const loford = indexfile.map(async (stak) => {
		const table = table_template(await skrifa(path.join('data', stak.csv)))
		const header = `<h1>${stak.title}</h1>
		<p>${stak.description}</p>`
		const htmlstrengur = template(stak.title, header, table)
